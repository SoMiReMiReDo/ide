...
def fetch_weather(location):
    ...
    print(location)
    print('{api}?{params}'.format(api=API, params=params))
...
def format_weather(city):
    ...
    # todo <WeatherJSONObject.WeatherJSONObject object at 0x000001C4FFDC1B50>

    # todo
    #      File "D:\PyClass\homework\LearnPythontheHardWay-master\extended\Chap3\project\weather_servers.py", line 47, in format_weather
    #     'wind_direction': weather_info.results[0].now.wind_direction}
    weather_data = {'city': weather_info.results[0].location.name,
                    'text': weather_info.results[0].now.text,
                    'temperature': weather_info.results[0].now.temperature
                    # 'wind_direction': weather_info.results[0].now.wind_direction
                    }
    # print("3")
    # print(weather_info.results[0].location.name)
